...
type HostCompliancePolicyRuleCondition struct {
	...
	Vulnerabilities []HostCompliancePolicyRuleVulnerability `json:"vulnerabilities"`
}
...
func CreateHostCompliancePolicy(c api.PrismaCloudComputeAPIClient, policy HostCompliancePolicy) error {
    fmt.Println("%%%%%%%%%%%%%%%%%%%%%%")
    fmt.Println("CreateHostCompliancePolicy request body:")
    fmt.Printf("%+v\n", policy)
    fmt.Printf("%+v\n", *policy.Rules)
    fmt.Println("%%%%%%%%%%%%%%%%%%%%%%")
...
func UpdateHostCompliancePolicy(c api.PrismaCloudComputeAPIClient, name string, policy HostCompliancePolicy) error {
	return c.Request(http.MethodPut, fmt.Sprintf("%s/%s", HostComplianceEndpoint, name), nil, policy, nil)
}

// Delete an existing collection.
func DeleteHostCompliancePolicy(c api.PrismaCloudComputeAPIClient, name HostCompliancePolicy) error {
	return c.Request(http.MethodDelete, fmt.Sprintf("%s/%s", HostComplianceEndpoint, name), nil, nil, nil)
}
